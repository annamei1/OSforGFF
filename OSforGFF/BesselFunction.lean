          ...
          have hbound' : ∀ t, 0 ≤ t →
            ...
            calc cosh t * exp (t - (z₀/2) * cosh t)
              ...
              _ ≤ exp t * exp (t - (z₀/2) * (exp t / 2)) := by
                  apply mul_le_mul_of_nonneg_left _ (exp_nonneg _)
                  apply exp_le_exp.mpr
                  have : -(z₀/2) * cosh t ≤ -(z₀/2) * (exp t / 2) :=
                    mul_le_mul_of_nonpos_left h_cosh_ge (by linarith)
                  linarith
              _ = exp (2 * t - (z₀/2) * exp t / 2) := by rw [← exp_add]; ring_nf
          have h_exp_to_zero : Tendsto
              (fun t ↦ exp (2 * t - (z₀/2) * exp t / 2)) atTop (nhds 0) := by
            apply tendsto_exp_atBot.comp
            ...
            have h_shifted : Tendsto
                (fun t : ℝ ↦ 4 / (z₀/2) * (t / exp t) - 1) atTop (nhds (-1)) := by
              convert h_scaled.sub_const 1 using 1; simp
            have h_prod : Tendsto (fun t : ℝ ↦
                ((z₀/2) * exp t / 2) * (4 / (z₀/2) * (t / exp t) - 1))
                atTop atBot :=
              Tendsto.atTop_mul_neg
                (by norm_num : (-1 : ℝ) < 0) hexp_inf h_shifted
            convert h_prod using 1; ext t; field_simp; ring
          have hpos : ∀ t, 0 ≤ cosh t * exp (t - (z₀/2) * cosh t) :=
...
lemma besselK1_asymptotic (z : ℝ) (hz : 1 ≤ z) :
    ...
    have h_Ioi : IntegrableOn f (Ioi 0) volume := by
      ...
      have h_ratio_tendsto : Tendsto (fun t ↦ f t / exp (-t)) atTop (nhds 0) := by
        ...
        have hbound : ∀ t, 0 ≤ t → cosh t * exp (t - z * cosh t) ≤ exp (2 * t - z * exp t / 2) := by
          intro t ht
          have h_cosh_le : cosh t ≤ exp t := by
            rw [cosh_eq]; linarith [exp_le_exp.mpr (by linarith : -t ≤ t)]
        ...
        have h_exp_to_zero : Tendsto (fun t ↦ exp (2 * t - z * exp t / 2)) atTop (nhds 0) := by
          ...
          have hexp_inf : Tendsto (fun t : ℝ ↦ z * exp t / 2) atTop atTop :=
            Tendsto.atTop_div_const (by linarith)
              (Tendsto.const_mul_atTop (by linarith : 0 < z)
                tendsto_exp_atTop)
          have h_ratio : Tendsto (fun t : ℝ ↦ t / exp t) atTop (nhds 0) := by
          ...
          have h_shifted : Tendsto (fun t : ℝ ↦ 4 / z * (t / exp t) - 1) atTop (nhds (-1)) := by
            convert h_scaled.sub_const 1 using 1; simp
          have h_prod : Tendsto (fun t : ℝ ↦
              (z * exp t / 2) * (4 / z * (t / exp t) - 1)) atTop atBot :=
            Tendsto.atTop_mul_neg
              (by norm_num : (-1 : ℝ) < 0) hexp_inf h_shifted
          convert h_prod using 1; ext t; field_simp; ring
        refine squeeze_zero' (Eventually.of_forall fun t ↦
          mul_nonneg (cosh_pos t).le (exp_nonneg _)) ?_ h_exp_to_zero
  ...
  have h_part2 : ∫ t in Ici 1, f t ≤ 2 * exp (-z) := by
    ...
    have hF_deriv : ∀ t, HasDerivAt F (g t) t := by
      ...
      have h2 : HasDerivAt (fun x ↦ exp (-z * exp x / 2))
  ...
  have h_disjoint : Disjoint (Ico (0:ℝ) 1) (Ici 1) := by
    ...
    simp only [mem_Ico] at hx; simp only [mem_Ici] at hx'; linarith
        ...
        have hbound : ∀ t, 0 ≤ t → cosh t * exp (t - z * cosh t) ≤ exp (2 * t - z * exp t / 2) := by
          ...
          calc cosh t * exp (t - z * cosh t)
            ...
            _ ≤ exp t * exp (t - z * (exp t / 2)) := by
                apply mul_le_mul_of_nonneg_left _ (exp_nonneg _)
                apply exp_le_exp.mpr
                linarith [mul_le_mul_of_nonpos_left h_cosh_ge
                  (by linarith : -z ≤ 0)]
            _ = exp (2 * t - z * exp t / 2) := by rw [← exp_add]; ring_nf
        have h_exp_to_zero : Tendsto (fun t ↦ exp (2 * t - z * exp t / 2)) atTop (nhds 0) := by
          apply tendsto_exp_atBot.comp
          ...
          have h_ratio : Tendsto (fun t : ℝ ↦ t / exp t) atTop (nhds 0) := by
            have h := tendsto_pow_mul_exp_neg_atTop_nhds_zero 1; simp only [pow_one] at h
            convert h using 1; ext t; rw [exp_neg, div_eq_mul_inv]
          have h_scaled : Tendsto
              (fun t : ℝ ↦ 4 / z * (t / exp t)) atTop (nhds 0) := by
            simpa using h_ratio.const_mul (4/z)
          have h_shifted : Tendsto (fun t : ℝ ↦ 4 / z * (t / exp t) - 1) atTop (nhds (-1)) := by
            convert h_scaled.sub_const 1 using 1; simp
          have h_prod : Tendsto (fun t : ℝ ↦
...
lemma integrableOn_radial_besselK1 (m : ℝ) (hm : 0 < m) :
  ...
  · -- Near origin: r² K₁(mr) ≤ r² * ((cosh 1 + 2)/(mr)) = (cosh 1 + 2)*r/m
    ...
    have hf_meas : AEStronglyMeasurable
        (fun r ↦ r ^ 2 * besselK1 (m * r))
        (volume.restrict (Ioc 0 (1/m))) := by
      -- besselK1 ∘ (m * ·) is continuous on (0, ∞)
  ...
  · -- At infinity: use besselK1_asymptotic for exponential decay
    ...
    have h_nonneg : ∀ r ∈ Ioi (1/m : ℝ),
        0 ≤ r ^ 2 * besselK1 (m * r) := by
      intro r hr
...
lemma bessel_symmetry_integral (z : ℝ) (hz : 0 < z) :
        ...
        have h_strict_mono : StrictMonoOn (fun y ↦ exp y - y^2 / 2) (Set.Ici 4) := by
          ...
          · intro x hx
            ...
            have hderiv : deriv (fun y ↦ exp y - y^2 / 2) x = exp x - x := by
              ...
              have hd2 : DifferentiableAt ℝ (fun y ↦ y^2 / 2) x :=
  ...
  -- 1. Split the integral over ℝ into (-∞, 0] and (0, ∞)
  -- using intervalIntegral.integral_Iic_add_Ioi
