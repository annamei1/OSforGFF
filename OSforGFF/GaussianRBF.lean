...
lemma innerProduct_isPositiveDefiniteKernel :
  ...
  -- First, expand star(c_i) * c_j = (a_i - ib_i)(a_j + ib_j)
  --   = (a_i a_j + b_i b_j) + i(a_i b_j - b_i a_j)
  have expand_cc : ∀ i j, star (c i) * c j =
      (a i * a j + b i * b j : ℝ) + Complex.I * (a i * b j - b i * a j : ℝ) := by
    intro i j
  ...
  have re_prod : ∀ i j,
      (star (c i) * c j * (⟪x i, x j⟫_ℝ : ℂ)).re =
        (a i * a j + b i * b j) * ⟪x i, x j⟫_ℝ := by
    intro i j
    rw [expand_cc]
    simp only [Complex.add_re, Complex.ofReal_re, Complex.mul_re, Complex.I_re, Complex.I_im,
...
lemma posSemidef_matrix_of_isPositiveDefiniteKernel {α : Type*} (K : α → α → ℂ)
...
lemma exp_isPositiveDefiniteKernel {α : Type*} (K : α → α → ℂ) (hK : IsPositiveDefiniteKernel K)
    (h_real : ∀ x y, (K x y).im = 0)
  ...
  -- Use OSforGFF.posSemidef_entrywiseExpHadamardSeries_of_posSemidef to show exp preserves PSD.
  intro m x c
  ...
  have hM_psd : M.PosSemidef := by
    exact posSemidef_matrix_of_isPositiveDefiniteKernel K h_real h_symm hK m x

  -- Apply the Hadamard series result: entrywiseExp M is PSD
  have hExpM_psd : (OSforGFF.entrywiseExp M).PosSemidef := by
    have h := OSforGFF.posSemidef_entrywiseExpHadamardSeries_of_posSemidef M hM_psd
  ...
  have h_sum_eq : (∑ i : Fin m, ∑ j : Fin m, star (c i) * c j * cexp (K (x i) (x j))).re =
    ...
    -- Similar to innerProduct_isPositiveDefiniteKernel proof
    simp only [Complex.re_sum]
...
theorem gaussian_rbf_isPositiveDefiniteKernel :
  ...
  -- 4. The RHS is ≥ 0 by exp_isPositiveDefiniteKernel + innerProduct_isPositiveDefiniteKernel.
  intro m x c
  ...
  have star_exp : ∀ i,
      star (cexp (-(1/2 : ℂ) * (‖x i‖^2 : ℝ))) =
        cexp (-(1/2 : ℂ) * (‖x i‖^2 : ℝ)) := by
    intro i
    rw [star_def]
  ...
  have h_sum :
      ∑ i : Fin m, ∑ j : Fin m,
        star (c i) * c j * cexp (-(1/2 : ℂ) * (‖x i - x j‖^2 : ℝ)) =
      ∑ i : Fin m, ∑ j : Fin m,
        star (d i) * d j * cexp (⟪x i, x j⟫_ℝ : ℂ) := by
    apply Finset.sum_congr rfl; intro i _
    apply Finset.sum_congr rfl; intro j _
    rw [factor, star_d]
    simp only [d]
  ...
  have h_sum' :
      ∑ i : Fin m, ∑ j : Fin m, (starRingEnd ℂ) (c i) * c j *
        cexp (-(1/2 : ℂ) * (‖x i - x j‖^2 : ℝ)) =
      ∑ i : Fin m, ∑ j : Fin m, (starRingEnd ℂ) (d i) * d j *
        cexp (⟪x i, x j⟫_ℝ : ℂ) := by
  ...
  have h_inner_pd : IsPositiveDefiniteKernel (fun (x y : E) ↦ (⟪x, y⟫_ℝ : ℂ)) :=
    innerProduct_isPositiveDefiniteKernel
  have h_exp_inner_pd : IsPositiveDefiniteKernel (fun (x y : E) ↦ cexp (⟪x, y⟫_ℝ : ℂ)) := by
    apply exp_isPositiveDefiniteKernel _ h_inner_pd
