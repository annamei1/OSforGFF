...
lemma glasser_lower_bound (c u : ℝ) (hu : u ≠ 0) : (c / u - u)^2 ≥ u^2 - 2*c := by
  rw [glasser_expand c u hu]
  have : 0 ≤ c^2 / u^2 := div_nonneg (sq_nonneg c) (sq_nonneg u)
  linarith
...
lemma glasser_integral_substitution_identity (c : ℝ) (hc : 0 < c) :
  ...
  have h_deriv : ∀ u ∈ Ioi 0, HasDerivWithinAt (fun u ↦ c / u) (-c / u^2) (Ioi 0) u :=
    fun u hu ↦
  ...
  rw [h_image] at h_cov
  have h_simp : ∀ u ∈ Ioi 0, |-c / u^2| * g (c / u) =
      (c / u^2) * exp (-(c/u - u)^2) := fun u hu ↦ by
    have hu_pos : 0 < u := hu
  ...
  · -- On (1, ∞): dominated by e^{2c} · e^{-u²}
    ...
    have h_contOn : ContinuousOn (fun u : ℝ ↦ exp (-(c/u - u)^2)) (Ioi 1) := by
      ...
      · exact continuousOn_const.div continuousOn_id
            (fun x hx ↦ ne_of_gt (lt_trans one_pos hx))
      · exact continuousOn_id
...
Proof: Use change of variables v = c/u which maps (0,1] to [c,∞) and (1,∞) to (0,c].
...
theorem glasser_weighted_integrable (c : ℝ) (hc : 0 < c) :
  ...
  · -- On (0, 1]: Use change of variables v = c/u, which maps (0, 1] → [c, ∞)
    ...
    --         = IntegrableOn (fun u => (c/u²) * exp(-(c/u - u)²)) (Ioc 0 1)
    --           [since (a-b)² = (b-a)²]
    ...
    -- Derivative of f
    have h_deriv : ∀ u ∈ Ioc (0 : ℝ) 1,
        HasDerivWithinAt (fun u ↦ c / u) (-c / u^2) (Ioc 0 1) u := by
      intro u hu
  ...
  · -- On (1, ∞): c/u² ≤ c, dominated by c · e^{2c} · e^{-u²}
    ...
    have h_contOn :
        ContinuousOn (fun u : ℝ ↦ (c/u^2) * exp (-(c/u - u)^2)) (Ioi 1) := by
      apply ContinuousOn.mul
      · exact continuousOn_const.div (continuousOn_pow 2)
            (fun x hx ↦ pow_ne_zero 2 (ne_of_gt (lt_trans one_pos hx)))
      · apply Real.continuous_exp.comp_continuousOn
        apply ContinuousOn.neg; apply ContinuousOn.pow
        apply ContinuousOn.sub
        · exact continuousOn_const.div continuousOn_id
...
lemma glasser_tendsto_atBot_at_top (c : ℝ) (_hc : 0 < c) :
  ...
  have h1 : Tendsto (fun u ↦ c / u) atTop (𝓝 0) :=
    Filter.Tendsto.const_div_atTop tendsto_id c
...
lemma glasser_strictAntiOn (c : ℝ) (hc : 0 < c) :
    StrictAntiOn (fun u ↦ c / u - u) (Ioi 0) := by
  intro x hx y hy hxy
...
lemma laplace_integral_factor (a b : ℝ) (ha : 0 < a) (hb : 0 < b) :
  ...
  refine setIntegral_congr_fun measurableSet_Ioi fun t ht ↦ ?_
  rw [show -a/t^2 - b*t^2 = -(a/t^2 + b*t^2) by ring,
    complete_square a b ha hb t ht, neg_add, exp_add]
  ring_nf
...
lemma laplace_integral_subst_scale (a b : ℝ) (ha : 0 < a) (hb : 0 < b) :
  ...
  have h := @integral_comp_mul_left_Ioi ℝ _ _
    (fun u ↦ exp (-(sqrt (a * b) / u - u)^2)) 0 (sqrt b) hsb
